...
- [ ] T103 新增 API 端點 `/api/catalog/import-txt.php`：許可權/CSRF 驗證、檔案檢查、呼叫匯入服務並回傳 JSON
...
- [x] T301 實作覆寫策略（預設跳過，提供引數決定是否覆寫），並在結果中列出跳過原因
- [x] T302 寫入 server log 或 activity log：記錄匯入者、型別、CSV 行為統計
- [x] T303 手動測試指令碼：1000 筆成功、包含錯誤行、分類自動建立、許可權/檔案大小檢查
