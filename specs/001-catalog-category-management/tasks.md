...
## Phase 2: Tests & Diagnostics
 ...
 - [X] T201 在 `quotes/index.php` 加入臨時除錯紀錄（error_log/console）以調查「加载报价单列表失败」
...
## Phase 4: Core - 類別管理介面
 ...
 - [X] T401 完成 `/categories/index.php` 類別樹管理（列表、建立、編輯、刪除、階層視圖、錯誤處理）
 - [X] T402 為類別管理導入一致的麵包屑、主題切換、底部導覽
...
## Phase 5: Core - 產品/服務表單整合
 ...
 - [X] T502 `products/edit.php` 類別選單載入與更新保存
 - [X] T503 `services/new.php` 與 `services/edit.php` 使用 `type='service'` 類別樹並保存類別
...
## Phase 6: Integration - 顯示與引用
 ...
 - [ ] T602 在報價建立與報價明細頁顯示引用項目的類別路徑
 - [ ] T603 更新匯出腳本（例如 `exports/export-products.php`）包含類別資訊

---

## Phase 7: Polish - 導航與樣式一致性

 - [ ] T701 全站麵包屑調整為 `标题 / 首页 › 当前页面` 格式
 - [ ] T702 移除產品/服務頁面多餘的切換按鈕，改用底部導覽
 - [ ] T703 調整報價頁面標題與資訊區塊（「首页 / › 报价管理」與日期等資訊）位置，避免重覆區塊
 - [ ] T704 修正報價單頁面 CSS 對齊問題，確保明暗主題都一致

---

## Phase 8: Polish - 文件與交付
...
## Phase 9: Completion
 ...
 - [ ] T901 手動驗證：建立/編輯產品與服務、指派類別、建立報價引用項目、確認顯示與匯出正確
 - [ ] T902 清理除錯紀錄與臨時代碼，檢查 `tasks.md` 對應任務標記為完成
