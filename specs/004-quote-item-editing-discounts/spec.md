...
作為業務，我希望在「編輯報價單」頁面就能調整每一個產品/服務行（更換專案、更新數量/單價/稅率），讓草稿不必刪除重建。
...
**Acceptance Scenarios**
1. **Given** 行有折扣，**When** 開啟報價單檢視頁，**Then** 表格中出現折扣列或折扣欄位。
2. **Given** 行無折扣，**When** 列印頁，**Then** 折扣欄留白或顯示 0，不破壞版面。

### Edge Cases
- 折扣輸入負數或非數字 → 系統應立即提示並阻止儲存。
- 折扣金額超過行小計 → 阻止提交並顯示「折扣不可超過行金額」。
- 已傳送/已接受報價單 → 維持只讀，不允許編輯明細。
...
### Success Criteria
- **SC-001**：草稿報價單可以在同一頁面完成明細調整且 100% 成功儲存。
